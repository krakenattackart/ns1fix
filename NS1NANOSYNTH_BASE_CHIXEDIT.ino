...
#include <frequencyToNote.h>  // Chix included MIDIUSB libraries
#include <MIDIUSB.h>
...
DAC_MCP49xx dac(DAC_MCP49xx::MCP4922, 4);   //NS1nanosynth has DAC SS on pin D4
...
MIDI_ MIDIUSB;  // Chix defined MIDI_ type MIDIUSB
...
void updateControl(){
  while(MIDIUSB.available() > 0) { 
      ...
      midiEventPacket_t e;    // Chix changed MIDIEVENT to midiEventPacket_t... because it didn't exist!
      e = MIDIUSB.read();
   //Serial.print("midi");
...
/*
  ...
  Chix changed type->header, m1->byte1, etc...

 */
   
      if(e.header == NOTEON) {  // Chix changed "e.type" to "e.header" cuz type was not in the struct
        if(e.byte1 == (0x90 + MIDI_CHANNEL)){ 
          if (e.byte2==23){
            //int DacOutB=DacVal[e.byte2-36];
	    //dac.outputB(DacOutB);	
            // set midi channel to 2
            MIDI_CHANNEL=1;
          }
          if(e.byte2 >= MIN_NOTE && e.byte2 <= MAX_NOTE){
            if(e.byte3==0)        //ATTENZIONE, GESTIONE NOTE OFF con NOTE ON e VELOCITY ZERO
              removeNote(e.byte2);
            else
              addNote(e.byte2);
              
          } else if (e.byte2 < MIN_NOTE) {
            ...
            if (e.byte2>=12 && e.byte2<=23){
              MIDI_CHANNEL=e.byte2-12;
            } 
          } else if (e.byte2 > MAX_NOTE) {
            // adjust oscillator adjustment with high C# and D#
            //if (e.byte2==85) oscAdjust--;
            //if (e.byte2==87) oscAdjust++;
          }
        }
      }
      
      if(e.header == NOTEOFF) {
        if(e.byte1 == 0x80 + MIDI_CHANNEL){
          removeNote(e.byte2);
        }
      }
      
      // set glide if portamento CC has moved
      if (e.header == CC && e.byte2 == 5){
        if (e.byte3 <= 3){
          // set glide to zero
          glide=0;
        } else {
          glide=map(e.byte3, 0, 127, 1, 2000);
          glide=1.0000/glide;
        }
      }
      
      // set modulation
      if (e.header == CC && e.byte2 == 1)
      {
        digitalWrite(13,1);
        digitalWrite(13,0);
        if (e.byte3 <= 3)
